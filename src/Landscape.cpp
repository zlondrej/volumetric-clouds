...
Landscape::Landscape(Camera *_camera) : camera(_camera), vao(0), vbo(0), ebo(0) {
    ...
    glBufferData(GL_ARRAY_BUFFER, 100 * 100 * sizeof (Vertex), NULL, GL_STREAM_DRAW);

    glGenBuffers(1, &ebo);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, ((LANDSCAPE_SIZE * 2) + 1) * LANDSCAPE_SIZE * sizeof (GLushort), NULL, GL_STATIC_DRAW);
    ...
    for (int row = 0; row < LANDSCAPE_SIZE; row++) {
        for (int col = 0; col < LANDSCAPE_SIZE; col++) {
            int i = (row + 1) * LANDSCAPE_SIZE + col;
            int j = row * LANDSCAPE_SIZE + col;
...
//    std::cout << "At: (" << atPosition.x << ", " << atPosition.y << ", " << atPosition.z << ")" << std::endl;
    ...
    glVertexAttribPointer(aPosition, 3, GL_FLOAT, GL_FALSE, sizeof (Vertex) - sizeof (vec3), offsetof(Vertex, position));

    // Normal attribute

    glEnableVertexAttribArray(aColor);
    glVertexAttribPointer(aColor, 3, GL_UNSIGNED_BYTE, GL_FALSE, sizeof (Vertex) - sizeof (u8vec3), (GLvoid*) offsetof(Vertex, color));

    glBindVertexArray(vao);

    glEnable(GL_PRIMITIVE_RESTART_FIXED_INDEX);
    glDrawElements(GL_TRIANGLE_STRIP, LANDSCAPE_SIZE * (LANDSCAPE_SIZE - 2), GL_UNSIGNED_SHORT, NULL);
...
void Landscape::step(float dt) {
    ...
    for (int row = 0; row < LANDSCAPE_SIZE; row++) {
        for (int col = 0; col < LANDSCAPE_SIZE; col++) {
            ...
            v->position = vec3(row - (LANDSCAPE_SIZE / 2), sin(float(row + col)), col - (LANDSCAPE_SIZE / 2));
